    ...
    <div class="container mx-auto px-4">
        ...
        <p class="text-indigo-200">
            @if(isset($jam['startdate']) && isset($jam['enddate']))
                @php
                    $startDate = \Carbon\Carbon::parse($jam['startdate']);
                    $endDate = \Carbon\Carbon::parse($jam['enddate']);
                @endphp
...
@php
    ...
    if($winners->isNotEmpty()) {
        foreach($winners as $entry) {
            ...
            $gameSlug = \Illuminate\Support\Str::slug($gameName);
                ...
                @if(isset($winnersByCategory[$category]))
                    @foreach($winnersByCategory[$category] as $winner)
                        <p class="text-gray-700 dark:text-gray-300">
                            Winner in the category <strong class="font-semibold dark:text-white">{{ $categoryNames[$category] ?? ucfirst($category) }}</strong>: 
                            <a href="/{{ $year }}/{{ $winner['slug'] }}" class="text-indigo-600 dark:text-indigo-400 hover:underline font-semibold">{{ $winner['game'] }}</a> 
                            by {{ $winner['team'] }}
                        </p>
                    @endforeach
                @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="games-grid">
            ...
            <div class="game-card {{ $playerClass }} {{ $controlClasses }} bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                <div class="relative h-48 bg-gray-200 dark:bg-gray-700 overflow-hidden">
                    ...
                    <img src="/media/{{ $year }}/{{ $headerImage }}" alt="{{ $gameName }}" class="w-full h-full object-cover">
                    @else
                    ...
                    <div class="absolute top-4 right-4">
                        <span class="text-white px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-1" style="background-color: #fbbf24;">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 512.00099 512" xmlns="http://www.w3.org/2000/svg">
